...
void search_test(HashTable table, float *resultsArr, const int *dataArr, int elem_loaded, int store_index, int *searchCollisionsArr)
{
    int rand_index;
    float search_time = 0;
    ...
    for(int i = 0; i < 100; i++)
    {
        ...
        table.searchItem(dataArr[rand_index]);
    ...
    searchCollisionsArr[store_index] = table.getNumOfSearchCollision(); //store search collisions
}
...
void test(const int* data, string outputFile)//tests inserts and can pass off to search
{
    ...
    HashTable table(40009); //initialize linked list
    ...
    for(int i = 0; i < 400; i++) //note to self: leave inequalities alone, tested
    {
        for(int j = 0; j < 100; j++)
        {
            ...
            table.insertItem(data[counter]); //insert
            //cout << "ran" << endl;
            end = std::chrono::high_resolution_clock::now();
            time_span = chrono::duration_cast<chrono::duration<float>>(end-start);
        ...
        insertCollisions[i] = table.getNumOfInsertCollision();
