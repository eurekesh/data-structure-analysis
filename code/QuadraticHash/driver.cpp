...
void search_test(QuadraticHash table, float *resultsArr, const int *dataArr, int elem_loaded, int store_index, int *searchCollisionsArr)
{
    int rand_index;
    float search_time = 0;
...
void test(const int* data, string outputFile)//tests inserts and can pass off to search
{
    ...
    QuadraticHash table; //initialize linked list
