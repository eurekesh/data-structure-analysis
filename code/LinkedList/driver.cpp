...
void search_test(LL linked, float *resultsArr, const int *dataArr, int elem_loaded, int store_index)
{
  int rand_index;
  float search_time;
  ...
  for(int i = 0; i < 100; i++)
  {
    rand_index = rand() % elem_loaded; // rand [0, elem_loaded)

    start = std::chrono::high_resolution_clock::now();

    int result = linked.search(dataArr[rand_index]);
...
void test(const int* data, string outputFile)//tests inserts and can pass off to search
{
  ...
  LL linkedList; //initialize linked list
  ...
  for(int i = 0; i < 400; i++) //note to self: leave inequalities alone, tested
  {
    for(int j = 0; j < 100; j++)
    {
      ...
      linkedList.insert(data[counter]); //insert
    ...
    search_test(linkedList, search, data, counter, i);
  }
  //linkedList.display();
...
int main(int argc, char const *argv[]){
  if(strcmp(argv[2], "a") == 0|| strcmp(argv[2], "A") == 0) //strcmp because pointers
  {
